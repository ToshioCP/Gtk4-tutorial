...
class Test_lib_src_file < Minitest::Test
  ...
  def test_add_head_tail_html
    ...
    However, this tutorial describes only <em>C programs on Linux</em>.</p>
    EOS
    expected_sec1_html = <<~'EOS'
    <!DOCTYPE html>
    ...
    File.write("#{directory}/sec1.html", sec1_html)
    add_head_tail_html("#{directory}/sec1.html")
    actual_sec1_html = File.read("#{directory}/sec1.html")
    remove_entry_secure(directory)
    # If you want to see the difference
    # Diff::LCS.diff(expected_sec1_html.each_line.to_a, actual_sec1_html.each_line.to_a).each do |array|
    #   array.each do |change|
    #     print "#{change.action}  #{change.position.to_s.chomp}: #{change.element.to_s.chomp}\n"
    #   end
    # end
    assert_equal expected_sec1_html, actual_sec1_html
  end
end
