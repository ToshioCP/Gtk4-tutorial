...
class Test_mktabl < Minitest::Test
  def test_mktbl
    mktbl = "../src/mktbl.rb"
    ...
    if File.exist? "test.txt.bak"
      tbl_backup = File.read "test.txt.bak"
      File.delete "test.txt.bak"
    else
      tbl_backup = nil
    end
    refute_nil tbl_generated, "Mktbl.rb removed or renamed test.txt."
    assert_equal tbl_expected, tbl_generated
    refute_nil tbl_backup, "Mktbl.rb didn't copy test.txt into test.txt.bak."
    assert_equal tbl, tbl_backup
  end
end
