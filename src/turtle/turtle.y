 ...
 There are two phases to run the program.
  ...
  1. parsing and generate a tree.
       Node is a joint to connect children's node to itself.
       There are two types of nodes, non-terminal and terminal.
...
show_symbol_table (void) {
  int i;
  g_printerr ("****** Symbol table ******\n");
  g_printerr ("  Table pointer (tp) = %d.\n", tp);
  g_printerr ("  ** Contents **\n");
  for (i=0; i<tp; ++i)
    if (table[i].type == PROC)
      g_printerr ("PROC,  %s,  %lx\n", table[i].name, (long int) table[i].object.node);
    else
...
#ifdef debug
g_printerr ("eval: node type is %s.\n", node_type_table[node->type]);
#endif
...
#ifdef debug
g_printerr ("execute: node type is %s.\n", node_type_table[node->type]);
g_printerr ("----- Current status -----\n");
if (pen)
  g_printerr ("    Pen down.\n");
else
  g_printerr ("    Pen up.\n");
g_printerr ("    Angle from x axis is %f.\n", angle);
g_printerr ("    Current X coordinate is %f.\n", cur_x);
g_printerr ("    Current Y coordinate is %f.\n", cur_y);
g_printerr ("    Line width is %f.\n", line_width);
g_printerr ("    Background color is (%f, %f, %f).\n", bc.red, bc.green, bc.blue);
g_printerr ("    Foreground color is (%f, %f, %f).\n", fc.red, fc.green, fc.blue);
show_symbol_table();
show_stack();
#endif
  ...
  switch (node->type) {
    ...
    case N_FD:
      ...
      cur_y += y;
#ifdef debug
g_printerr ("fd: Distance is %f.\n", d);
g_printerr ("fd: X-distance is %f.\n", x);
g_printerr ("fd: Y-distance is %f.\n", y);
g_printerr ("fd: New X coordinate is %f.\n", cur_x);
g_printerr ("fd: New Y coordinate is %f.\n", cur_y);
#endif
...
#define fixcolor(c)  c = c < 0 ? 0 : (c > 1 ? 1 : c)
    ...
    case N_FC:
      ...
      fixcolor (fc.blue);
#ifdef debug
g_printerr ("fc:  Foreground color is (%f, %f, %f).\n", fc.red, fc.green, fc.blue);
#endif
      break;
    case N_ASSIGN:
...
run (void) {
  ...
  if (! init_cairo()) {
    g_printerr ("Cairo not initialized.\n");
    return;
  }
  init_table();
  init_stack();
...
#ifdef debug
  /* statistics */
  g_printerr ("------ Statistics ------\n");
  g_printerr ("The biggest depth of the symbol table is %d.\n", tp);
  g_printerr ("The biggest depth of the stack is %d.\n", sp_biggest);
#endif
  destroy_cairo ();
