...
To generate the tutorial documents in your local repository, the following environment and software are required. I have also included the versions used in my own environment for reference.
...
- Ruby, Rake, Pandoc, and LaTeX: These are used to generate the documentation.
  - Ruby: Version 4.0.1 is used. Ruby version 3 or later may work, but you may need to modify the Gemfile, .ruby-version, or mise.toml.
...
@@@table command will be explained in a later section. 

### The @@@if Command

This command begins with `@@@if` , optionally followed by `@@@elif` or `@@@else`, and ends with `@@@end`.
These wotk similarly to the C preprocessor directives `#if`, `#elif`, `#else`, and `#endif`.
...
A list of supported languages is defined in `/data/ext.yml`.
...
It is replaced by the command line, its standard output, and the subsequent prompt.
...
- Relative URL: A path relative to the current URL. If the current page is `https://example.com/a/d.html`, the relative URL to `https://example.com/a/b/c.html` would be `b/c.html`. Note that relative URL doesn't start with slash.
...
For internal links, any of the three types can be used, but root-relative or relative URLs are preferred.
This ensures the links remain valid even if the domain name changes.

When writing root-relative URLs in .src.md files, the `src` directory should be treated as the site root.
The Ruby script automatically adjusts these links during the transformation to the target format (GFM or HTML).
When converting to PDF, internal links within the repository are removed, except for those automatically generated by Pandoc (such as table of contents links).

The following usage of root-relative or relative URLs in .src.md files is recommended:

- Root-relative URLs: All image files that are stored in the `/images` directory (`/` is the `src` directory).
- Relative URLs: Links to section files or other .src.md files

Links to C source files can be either of root-relative or ralative links.

- Root-relative URLs are suitable for files whose location relative to the src directory is unlikely to change, such as C source files in a fixed global location.
- Relative URLs are more appropriate when the .src.md document and its related C source files are part of a self-contained project (e.g., a "turtle" graphics sample). This ensures the links remain intact even if the entire project directory is moved.

**Example:** if you have an image at `/images/a.png` and a section file at `/sec1.src.md`, and you want to link to them from `/sec2.src.md`:

```
Link to an image: ![Image a](/images/a.png)
Link to a section: [Section 1](sec1.src.md)
```

**Path Validation:**

Ensure that all internal links remain within the repository boundaries.
The Ruby script will detect and warn you if a path (e.g., ../../../etc/passwd) attempts to point outside the project root.
...
Additionally, size specification in cm is intended for LaTeX conversion (the intermediate step for PDF) and is not for HTML.
These attributes are preserved only when the target is pdf; the Ruby script removes them for other targets.

Because the Ruby script handles link conversion automatically, there is no need to use @@@if to differentiate link formats between targets.

### Other Notes on Writing .src.md

The Ruby script assumes that the starting and ending lines of a fenced code block consist of three or more identical characters (backticks or tildes).
If these are not written correctly, the script will fail to recognize the boundaries of the code.
...
As explained later, section files (`sec<num>.src.md`) must be placed directly under the source directory root.
However, other .src.md files can be organized in subdirectories within the source directory.
This directory structure is mirrored in the target directory.
