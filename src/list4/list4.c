...
app_startup (GApplication *application) {
  ...
  GtkListItemFactory *factory_list;
  GtkListItemFactory *factory_grid;
  GFile *file;
  ...
  factory_list = gtk_builder_list_item_factory_new_from_resource (NULL, "/com/github/ToshioCP/list4/factory_list.ui");
  gtk_list_view_set_factory (list, factory_list);
  g_object_unref (factory_list);
  g_signal_connect (list, "activate", G_CALLBACK (list_activate), NULL);

  factory_grid = gtk_builder_list_item_factory_new_from_resource (NULL, "/com/github/ToshioCP/list4/factory_grid.ui");
  gtk_grid_view_set_factory (grid, factory_grid);
  g_object_unref (factory_grid);
  ...
  gtk_css_provider_load_from_string (provider, "button#btnlist {background: silver;} button#btngrid {background: white;}");
